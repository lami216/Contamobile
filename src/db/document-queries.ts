...
function mapDocument(row:DocumentRow,lines:DocumentLine[]=[]):DocumentRecord{return{id:row.id,number:row.number,sequence:row.sequence,kind:row.kind,status:row.status,partyId:row.party_id,partyName:row.party_name,warehouseId:row.warehouse_id,warehouseName:row.warehouse_name,destinationWarehouseId:row.destination_warehouse_id,destinationWarehouseName:row.destination_warehouse_name,paymentMethod:row.payment_method,fromAccountId:row.from_account_id,toAccountId:row.to_account_id,accountAdjustmentDirection:row.account_adjustment_direction,note:row.note,title:row.title,total:row.total,dueTotal:row.due_total,paidTotal:row.paid_total,cashAmount:row.cash_amount,partyCashDirection:row.party_cash_direction,partyBalanceBefore:row.party_balance_before,partyBalanceDelta:row.party_balance_delta,partyBalanceAfter:row.party_balance_after,businessDate:row.business_date,dailySequence:row.daily_sequence,pricingMode:row.pricing_mode,occurredAt:row.occurred_at,updatedAt:row.updated_at,revision:row.revision,voidedAt:row.voided_at,lines}}
