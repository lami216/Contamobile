...
].filter((item):item is NonNullable<typeof item>=>item!==null);return <Screen scroll><PageHeader title={t('sales')} subtitle={ar?'المبيعات والفواتير':'Ventes et facturation'}/>{items.length?<FeatureMenu items={items}/>:<EmptyState title={ar?'لا توجد وظائف متاحة لحسابك':'Aucune fonction disponible pour ce compte.'}/>}</Screen>}
