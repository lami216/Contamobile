...
export function ReportsScreen(){
  ...
  const load=useCallback(async()=>{
    ...
    if(issue){setReportError(rangeMessage(issue,ar));setData({metrics:[],rows:[]});return}
    try{setData(await runReport(db,type,from,to,filters));setReportError('')}catch(error){setReportError(error instanceof Error&&error.message.startsWith('invalid-report-range:')?rangeMessage(error.message.slice('invalid-report-range:'.length) as DateRangeIssue,ar):(ar?'تعذر إنشاء التقرير.':'Impossible de générer le rapport.'))}
  ...
  const label=(key:string)=>metricLabels[key]?.[locale]??key,filteredProducts=categoryId?products.filter(p=>p.categoryId===categoryId):products,selectedProduct=products.find(p=>p.id===productId),selectedParty=parties.find(p=>p.id===partyId),numericMetrics=new Set(['count','quantity','movements','netChange']);
  ...
  const openSource=(item:ReportData['rows'][number])=>{
    ...
    if(type==='product-sales'){router.push({pathname:'/inventory/products',params:{productId:item.id}});return}
    if(type==='debts'){router.push({pathname:'/parties/[id]',params:{id:item.id}})}
  };
  const canOpenSource=(item:ReportData['rows'][number])=>Boolean(item.sourceDocumentId||type==='product-sales'||type==='debts');
