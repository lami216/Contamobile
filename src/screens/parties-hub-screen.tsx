...
].filter((item):item is NonNullable<typeof item>=>item!==null);return <Screen scroll><PageHeader title={t('parties')} subtitle={ar?'العملاء والموردون':'Clients et fournisseurs'}/>{items.length?<FeatureMenu items={items}/>:<EmptyState title={ar?'لا توجد وظائف متاحة لحسابك':'Aucune fonction disponible pour ce compte.'}/>}</Screen>}
