...
export function BrandingScreen(){
  const db=useSQLiteContext(),auth=useAuth(),{t,locale}=useI18n();
  ...
  const save=async()=>{setBusy(true);try{await saveInvoiceBranding(db,form);Alert.alert(t('success'));router.back()}catch(error){Alert.alert(t('error'),error instanceof Error?error.message:t('error'))}finally{setBusy(false)}};
