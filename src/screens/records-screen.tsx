...
function DocumentModal({item,onClose,onChanged}:{item:DocumentRecord;onClose:()=>void;onChanged:()=>Promise<void>}){
  ...
  useEffect(()=>{let live=true;if(item.kind==='payment'&&item.partyId)void getParty(db,item.partyId).then(party=>{if(live)setPartyType(party?.partyType??null)});return()=>{live=false}},[db,item.kind,item.partyId]);
  const paymentEdit=partyType==='supplier'?auth.has('suppliers.pay.edit'):partyType==='customer'?auth.has('customers.collect.edit'):false;
  ...
  const canEdit=item.status==='posted'&&((item.kind==='sale'&&auth.has('pos.edit'))||(item.kind==='purchase'&&auth.has('purchases.edit'))||(item.kind==='transfer'&&auth.has('warehouses.transfer.edit'))||(item.kind==='adjustment'&&auth.has('warehouses.adjust.edit'))||(item.kind==='payment'&&paymentEdit));
  ...
  const edit=()=>{if(item.kind==='sale'||item.kind==='purchase'){onClose();router.push({pathname:'/sales/edit/[id]',params:{id:item.id,kind:item.kind}})}else if(item.kind==='transfer'){onClose();router.push({pathname:'/inventory/transfer',params:{documentId:item.id}})}else if(item.kind==='adjustment'){onClose();router.push({pathname:'/inventory/adjustment',params:{documentId:item.id}})}};
  const runVoid=()=>Alert.alert(ar?'إلغاء المستند':'Annuler le document',ar?'سيتم عكس أثر هذا المستند على المخزون والديون والحساب المالي.':'Les effets sur le stock, les dettes et le compte financier seront annulés.',[{text:t('cancel'),style:'cancel'},{text:t('confirm'),style:'destructive',onPress:()=>void (async()=>{try{if(item.kind==='sale'||item.kind==='purchase')await voidInvoice(db,item.kind,item.id);else if(item.kind==='expense')await voidExpense(db,item.id);else if(item.kind==='payment')await voidPartyCash(db,item.id);else if(item.kind==='transfer')await voidStockTransfer(db,item.id);else if(item.kind==='adjustment')await voidStockAdjustment(db,item.id);else if(item.kind==='account-transfer')await voidAccountTransfer(db,item.id);else if(item.kind==='account-adjustment')await voidAccountAdjustment(db,item.id);await onChanged()}catch(error){Alert.alert(t('error'),errorMessage(error))}})()}]);
