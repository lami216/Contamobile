...
export function RecordsScreen(){
  ...
  return <Screen padded={false}>
    <FlatList data={items} keyExtractor={item=>item.id} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.list} ListHeaderComponent={<View style={styles.header}>
      ...
      <Card tone="muted" style={styles.filterSummary}><View style={[styles.summaryLine,{flexDirection:isRTL?'row-reverse':'row'}]}><View><AppText variant="caption" muted>{ar?'الفترة الحالية':'Période active'}</AppText><AppText variant="subheading">{periodLabel}</AppText></View><Badge label={ar?`${items.length} مستند`:`${items.length} documents`} tone="primary"/></View></Card>
...
const styles=StyleSheet.create({
  ...
  filterSummary:{shadowOpacity:0,elevation:0},
  summaryLine:{alignItems:'center',justifyContent:'space-between',gap:spacing.md},
  ...
  detailTitle:{alignItems:'center',gap:spacing.md},
  summary:{justifyContent:'space-between',alignItems:'center',gap:spacing.sm,paddingVertical:spacing.xs},
