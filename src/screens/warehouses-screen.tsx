...
import { AppText, Button, Card, EmptyState, Field, PageHeader, Row, Screen, SectionTitle } from '@/components/ui';
...
export function WarehousesScreen(){
  ...
  return <Screen><PageHeader title={t('warehouses')} subtitle={ar?'إدارة المخازن وتحديد مخزن البيع الافتراضي':'Gestion des dépôts et du dépôt de vente par défaut'} action={canCreate?<Button title={t('add')} onPress={()=>setEditing(null)}/>:undefined}/><FlatList data={items} keyExtractor={x=>x.id} ListEmptyComponent={<EmptyState title={t('noData')}/>} renderItem={({item})=><Card elevated><Row title={item.name} subtitle={item.isSalesDefault?(ar?'مخزن البيع الافتراضي':'Dépôt de vente par défaut'):undefined} trailing={!item.isSalesDefault&&canEdit?<Button title={t('confirm')} variant="secondary" onPress={()=>void run(()=>setDefaultWarehouse(db,item.id))}/>:item.isSalesDefault?<AppText>✓</AppText>:undefined} onPress={canEdit?()=>setEditing(item):undefined}/></Card>}/>{editing!==undefined?<WarehouseEditor item={editing} onClose={()=>setEditing(undefined)} onSave={name=>run(async()=>{if(editing){if(!canEdit)throw new Error(ar?'ليس لديك صلاحية تعديل المخازن':'Vous n’avez pas le droit de modifier les dépôts.');return renameWarehouse(db,editing.id,name)}if(!canCreate)throw new Error(ar?'ليس لديك صلاحية إنشاء المخازن':'Vous n’avez pas le droit de créer des dépôts.');return createWarehouse(db,name)})} onArchive={editing&&!editing.isSalesDefault&&canDelete?()=>Alert.alert(t('delete'),editing.name,[{text:t('cancel'),style:'cancel'},{text:t('confirm'),style:'destructive',onPress:()=>void run(()=>archiveWarehouse(db,editing.id))}]):undefined}/>:null}</Screen>;
}
