...
export function WarehousesScreen(){
  const db=useSQLiteContext(),{t}=useI18n(),auth=useAuth();
  ...
  if(!auth.has('warehouses.view'))return <Screen><EmptyState title="ليس لديك صلاحية عرض المخازن"/></Screen>;
  const canCreate=auth.has('warehouses.create'),canEdit=auth.has('warehouses.edit'),canDelete=auth.has('warehouses.delete');
  const run=async(work:()=>Promise<unknown>)=>{try{await work();setEditing(undefined);await load()}catch(error){Alert.alert(t('error'),error instanceof Error?error.message:t('error'))}};
  return <Screen><SectionTitle title={t('warehouses')} action={canCreate?<Button title={t('add')} onPress={()=>setEditing(null)}/>:undefined}/><FlatList data={items} keyExtractor={x=>x.id} ListEmptyComponent={<EmptyState title={t('noData')}/>} renderItem={({item})=><Row title={item.name} subtitle={item.isSalesDefault?'مخزن البيع الافتراضي':undefined} trailing={!item.isSalesDefault&&canEdit?<Button title={t('confirm')} variant="secondary" onPress={()=>void run(()=>setDefaultWarehouse(db,item.id))}/>:item.isSalesDefault?<AppText>✓</AppText>:undefined} onPress={canEdit?()=>setEditing(item):undefined}/>}/>{editing!==undefined?<WarehouseEditor item={editing} onClose={()=>setEditing(undefined)} onSave={name=>run(async()=>{if(editing){if(!canEdit)throw new Error('ليس لديك صلاحية تعديل المخازن');return renameWarehouse(db,editing.id,name)}if(!canCreate)throw new Error('ليس لديك صلاحية إنشاء المخازن');return createWarehouse(db,name)})} onArchive={editing&&!editing.isSalesDefault&&canDelete?()=>Alert.alert(t('delete'),editing.name,[{text:t('cancel'),style:'cancel'},{text:t('confirm'),style:'destructive',onPress:()=>void run(()=>archiveWarehouse(db,editing.id))}]):undefined}/>:null}</Screen>;
}
