...
].filter((item):item is NonNullable<typeof item>=>item!==null);return <Screen scroll><PageHeader title={t('more')} subtitle={ar?'الإدارة والتقارير':'Gestion et rapports'}/>{items.length?<FeatureMenu items={items}/>:<EmptyState title={ar?'لا توجد وظائف متاحة لحسابك':'Aucune fonction disponible pour ce compte.'}/>}</Screen>}
