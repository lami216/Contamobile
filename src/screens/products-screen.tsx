...
export function ProductsScreen(){
  ...
  return <Screen padded={false}><FlatList data={items} keyExtractor={item=>item.id} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.list} ListHeaderComponent={<View style={styles.header}>
    ...
    {!showArchived?<View style={[styles.summary,{flexDirection:isRTL?'row-reverse':'row'}]}><Card tone="primary" style={styles.summaryCard}><AppText variant="caption" muted>{ar?'الكمية الكلية':'Quantité totale'}</AppText><AppText variant="heading">{number(summary.stock)}</AppText></Card><Card tone={summary.low>0?'warning':'muted'} style={styles.summaryCard}><AppText variant="caption" muted>{t('lowStock')}</AppText><AppText variant="heading" style={summary.low>0?styles.warning:undefined}>{number(summary.low)}</AppText></Card><Card tone={summary.expiry>0?'warning':'muted'} style={styles.summaryCard}><AppText variant="caption" muted>{ar?'صلاحية تحتاج انتباه':'Péremption à surveiller'}</AppText><AppText variant="heading" style={summary.expiry>0?styles.warning:undefined}>{number(summary.expiry)}</AppText></Card></View>:null}
...
function ProductEditor({product,warehouses,busy,onClose,onSave,onArchive}:{product:Product|null;warehouses:Warehouse[];busy:boolean;onClose:()=>void;onSave:(input:{name:string;barcode:string;pieceCost:number|null;piecePrice:number|null;wholesalePrice:number|null;expiryDate:string|null;note:string|null;openingStock?:number;openingWarehouseId?:string})=>Promise<void>;onArchive?:()=>void}){
  ...
  return <Modal animationType="slide" onRequestClose={onClose}><Screen padded={false}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.modal}>
    ...
    <Card><SectionTitle title={ar?'هوية المنتج':'Identité'}/><Field label={t('name')} value={form.name} onChangeText={set('name')} autoFocus={!product}/><Field label={t('barcode')} value={form.barcode} onChangeText={set('barcode')} autoCapitalize="none"/></Card>
    <Card><SectionTitle title={ar?'الأسعار':'Prix'}/><View style={[styles.pair,{flexDirection:isRTL?'row-reverse':'row'}]}><Field label={t('purchasePrice')} value={form.pieceCost} onChangeText={set('pieceCost')} keyboardType="number-pad" containerStyle={styles.flex}/><Field label={t('salePrice')} value={form.piecePrice} onChangeText={set('piecePrice')} keyboardType="number-pad" containerStyle={styles.flex}/></View><Field label={t('wholesalePrice')} value={form.wholesalePrice} onChangeText={set('wholesalePrice')} keyboardType="number-pad"/></Card>
    <Card><SectionTitle title={ar?'تفاصيل إضافية':'Détails supplémentaires'}/><Field label={t('expiryDate')} value={form.expiryDate} onChangeText={set('expiryDate')} placeholder="YYYY-MM-DD"/><Field label={t('note')} value={form.note} onChangeText={set('note')} multiline numberOfLines={3}/></Card>
    {!product?<Card tone="primary"><SectionTitle title={t('openingBalance')} subtitle={ar?'اختياري: أدخل الكمية الموجودة حاليًا إذا كنت تبدأ بمنتج قديم.':'Optionnel : saisissez le stock existant pour un produit déjà en magasin.'}/><Field label={t('quantity')} value={form.openingStock} onChangeText={set('openingStock')} keyboardType="decimal-pad"/><View style={[styles.chips,{flexDirection:isRTL?'row-reverse':'row'}]}>{warehouses.map(w=><Chip key={w.id} label={w.name} active={form.openingWarehouseId===w.id} onPress={()=>set('openingWarehouseId')(w.id)}/>)}</View></Card>:null}
...
const styles=StyleSheet.create({
  ...
  summary:{gap:spacing.sm,flexWrap:'wrap'},
