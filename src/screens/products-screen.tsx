...
export function ProductsScreen(){
  ...
  const saveCategory=async()=>{if(!categoryDraft.trim()||busy)return;setBusy(true);try{if(categoryEditing){if(!canEdit)throw new Error(ar?'ليس لديك صلاحية تعديل فئات المنتجات.':'Vous n’avez pas le droit de modifier les catégories.');await renameProductCategory(db,categoryEditing.id,categoryDraft)}else{if(!canCreate)throw new Error(ar?'ليس لديك صلاحية إنشاء فئات المنتجات.':'Vous n’avez pas le droit de créer des catégories.');await createProductCategory(db,categoryDraft)}setCategoryDraft('');setCategoryEditing(null);await load()}catch(error){Alert.alert(t('error'),errorMessage(error))}finally{setBusy(false)}};
  const removeCategory=(category:ProductCategory)=>{if(!canDelete){Alert.alert(t('error'),ar?'ليس لديك صلاحية حذف فئات المنتجات.':'Vous n’avez pas le droit de supprimer les catégories.');return}Alert.alert(ar?'حذف الفئة':'Supprimer la catégorie',ar?`سيتم حذف «${category.name}» فقط، وتبقى المنتجات بدون فئة.`:`« ${category.name} » sera supprimée, les produits resteront sans catégorie.`,[{text:t('cancel'),style:'cancel'},{text:t('confirm'),style:'destructive',onPress:()=>void(async()=>{setBusy(true);try{await deleteProductCategory(db,category.id);if(categoryId===category.id)setCategoryId('');if(categoryEditing?.id===category.id){setCategoryEditing(null);setCategoryDraft('')}await load()}catch(error){Alert.alert(t('error'),errorMessage(error))}finally{setBusy(false)}})()}])};
  return <Screen padded={false}><FlatList data={items} keyExtractor={item=>item.id} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.list} ListHeaderComponent={<View style={styles.header}>
    ...
    <View style={[styles.filters,{flexDirection:isRTL?'row-reverse':'row'}]}><Chip label={ar?'النشطة':'Actifs'} active={!showArchived} onPress={()=>setShowArchived(false)}/><Chip label={ar?'المؤرشفة':'Archivés'} active={showArchived} onPress={()=>setShowArchived(true)}/>{canManageCategories?<Button compact title={ar?'إدارة الفئات':'Catégories'} variant="ghost" onPress={()=>setCategoryManager(true)}/>:null}</View>
