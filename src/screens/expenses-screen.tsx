...
export function ExpensesScreen(){
  ...
  return <Screen padded={false}><FlatList data={items} keyExtractor={item=>item.id} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.list} ListHeaderComponent={<View style={styles.header}>
    ...
    <Card tone="primary" style={styles.hero}><AppText variant="caption" muted>{todayOnly?(ar?'مصاريف اليوم':'Dépenses du jour'):(ar?'مجموع النتائج':'Total affiché')}</AppText><Money value={total} tone="negative" large/><AppText variant="caption" muted>{ar?`${items.length} عملية`:`${items.length} opérations`}</AppText></Card>
