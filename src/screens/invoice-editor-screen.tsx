...
export function InvoiceEditorScreen({kind,documentId}:{kind:'sale'|'purchase';documentId?:string}){
  ...
  const submit=()=>{
    if(kind==='sale'&&!documentId){const check=validateSaleDraft(lines.map(line=>({productId:line.productId,quantity:line.quantity,piecePrice:line.unitPrice})),products,warehouseId);if(check.errors.length){const e=check.errors[0];Alert.alert(t('error'),e?.code==='insufficientQuantity'?(ar?`${e.productName}: المطلوب ${e.requested} والمتوفر ${e.available}`:`${e.productName} : demandé ${e.requested}, disponible ${e.available}`):e&&'productName'in e?e.productName:t('error'));return}if(check.warnings.length){Alert.alert(ar?'تنبيه':'Attention',check.warnings.map(w=>ar?`${w.productName}: سعر البيع ${w.salePrice} أقل من التكلفة ${w.purchaseCost}`:`${w.productName} : prix de vente ${w.salePrice} inférieur au coût ${w.purchaseCost}`).join('\n'),[{text:t('cancel'),style:'cancel'},{text:t('confirm'),onPress:()=>void persist()}]);return}}
    void persist();
  };
