...
export function AdjustmentScreen(){
  ...
  const [products,setProducts]=useState<Product[]>([]),[warehouses,setWarehouses]=useState<Warehouse[]>([]),[warehouseId,setWarehouseId]=useState(''),[reason,setReason]=useState(''),[lines,setLines]=useState<Line[]>([]),[picker,setPicker]=useState(false),[busy,setBusy]=useState(false),[editId,setEditId]=useState<string|null>(null),[actualDraft,setActualDraft]=useState('0');
  ...
  const submit=async()=>{if(busy||missingOrigin.length)return;setBusy(true);try{await adjustStock(db,{warehouseId,reason,lines:changed.map(line=>({productId:line.product.id,actualQuantity:line.actual}))});router.back()}catch(error){Alert.alert(t('error'),errorMessage(error))}finally{setBusy(false)}};
  ...
  </ScrollView><StickyActionBar label={t('confirm')} summary={changed.length?(ar?`${warehouseName} • ${changed.length} تغييرات`:`${warehouseName} • ${changed.length} changements`):undefined} loading={busy} disabled={!warehouseId||!reason.trim()||!changed.length||missingOrigin.length>0} onPress={()=>void submit()}/><ProductPicker visible={picker} products={products} exclude={lines.map(line=>line.product.id)} onClose={()=>setPicker(false)} onSelect={chooseProduct}/><Sheet visible={Boolean(editId)} title={ar?'الكمية الفعلية':'Quantité réelle'} onClose={()=>setEditId(null)} footer={<Button title={t('save')} onPress={saveEdit}/>}><Field label={t('actualQuantity')} value={actualDraft} onChangeText={setActualDraft} keyboardType="decimal-pad" autoFocus selectTextOnFocus/></Sheet></Screen>;
}
