...
].filter((item):item is NonNullable<typeof item>=>item!==null);return <Screen scroll><PageHeader title={t('inventory')} subtitle={ar?'المخزون والمنتجات':'Stock et produits'}/>{items.length?<FeatureMenu items={items}/>:<EmptyState title={ar?'لا توجد وظائف متاحة لحسابك':'Aucune fonction disponible pour ce compte.'}/>}</Screen>}
