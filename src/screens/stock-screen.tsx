...
export function StockScreen(){
  ...
  return <Screen padded={false}><FlatList
    ...
    ListHeaderComponent={<View style={styles.header}>
      ...
      <View style={[styles.metrics,{flexDirection:isRTL?'row-reverse':'row'}]}><MetricCard label={ar?'إجمالي الوحدات':'Unités totales'} value={summary.quantity}/><MetricCard label={t('lowStock')} value={summary.low} tone={summary.low>0?'negative':'normal'}/><MetricCard label={ar?'نفاد المخزون':'Ruptures'} value={summary.out} tone={summary.out>0?'negative':'normal'}/></View>
