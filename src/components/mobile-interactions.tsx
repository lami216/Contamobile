...
export function QuantityStepper({value,onDecrease,onIncrease,onEdit}:{value:number;onDecrease:()=>void;onIncrease:()=>void;onEdit?:()=>void}){
  ...
  const controls=<><Pressable accessibilityRole="button" accessibilityLabel="decrease" onPress={onDecrease} style={({pressed})=>[styles.stepButton,pressed&&styles.pressed]}><AppText variant="heading" style={styles.stepSymbol}>−</AppText></Pressable><Pressable accessibilityRole={onEdit?'button':undefined} onPress={onEdit} disabled={!onEdit} style={({pressed})=>[styles.stepValue,pressed&&onEdit&&styles.pressed]}><AppText variant="subheading" style={styles.stepNumber}>{number(value)}</AppText></Pressable><Pressable accessibilityRole="button" accessibilityLabel="increase" onPress={onIncrease} style={({pressed})=>[styles.stepButton,styles.stepButtonPrimary,pressed&&styles.pressed]}><AppText variant="heading" style={styles.stepPlus}>+</AppText></Pressable></>;
...
export function StickyActionBar({label,onPress,disabled=false,loading=false,summary,secondaryAction}:{label:string;onPress:()=>void;disabled?:boolean;loading?:boolean;summary?:string;secondaryAction?:ReactNode}){
  ...
  return <View style={[styles.bottomBar,{paddingBottom:Math.max(insets.bottom,spacing.sm)}]}><View style={[styles.bottomInner,{flexDirection:isRTL?'row-reverse':'row'}]}>{summary?<View style={styles.bottomSummary}><AppText variant="caption" muted>{summary}</AppText></View>:secondaryAction?<View style={styles.secondarySlot}>{secondaryAction}</View>:<View style={styles.bottomSummary}/>}<Pressable accessibilityRole="button" disabled={disabled||loading} onPress={onPress} style={({pressed})=>[styles.checkout,pressed&&styles.pressed,(disabled||loading)&&styles.disabled]}>{loading?<ActivityIndicator color={colors.onPrimary}/>:<AppText variant="subheading" style={styles.checkoutText}>{label}</AppText>}</Pressable></View></View>;
}
...
const styles=StyleSheet.create({
  pressed:{opacity:.68,transform:[{scale:.985}]},disabled:{opacity:.48},
  stepper:{alignItems:'center',borderRadius:radius.full,backgroundColor:colors.surfaceMuted,padding:3,gap:3},stepButton:{width:touch.min,height:touch.min,borderRadius:radius.full,alignItems:'center',justifyContent:'center',backgroundColor:colors.surface},stepButtonPrimary:{backgroundColor:colors.primarySoft},stepValue:{minWidth:46,height:touch.min,alignItems:'center',justifyContent:'center'},stepNumber:{fontVariant:['tabular-nums']},stepSymbol:{lineHeight:25,color:colors.text},stepPlus:{lineHeight:25,color:colors.primary},
  bottomBar:{backgroundColor:colors.surface,borderTopWidth:1,borderTopColor:colors.border,paddingTop:spacing.sm,paddingHorizontal:spacing.md,...elevation.floating},bottomInner:{alignItems:'center',gap:spacing.md},bottomSummary:{flex:1,gap:spacing.xxs},secondarySlot:{flex:1},checkout:{minHeight:58,minWidth:148,borderRadius:radius.lg,alignItems:'center',justifyContent:'center',paddingHorizontal:spacing.lg,backgroundColor:colors.primary},checkoutText:{color:colors.onPrimary},
  overlay:{flex:1,justifyContent:'flex-end',backgroundColor:colors.overlay},sheet:{maxHeight:'88%',backgroundColor:colors.surface,borderTopLeftRadius:radius.xl,borderTopRightRadius:radius.xl,paddingTop:spacing.xs,paddingHorizontal:spacing.md,...elevation.floating},handle:{width:42,height:5,borderRadius:99,backgroundColor:colors.borderStrong,alignSelf:'center',marginBottom:spacing.sm},sheetHead:{minHeight:touch.comfortable,alignItems:'center',justifyContent:'space-between',gap:spacing.md},close:{width:touch.min,height:touch.min,borderRadius:radius.full,alignItems:'center',justifyContent:'center',backgroundColor:colors.surfaceMuted},sheetBody:{gap:spacing.md,paddingVertical:spacing.md},sheetFooter:{paddingTop:spacing.sm,gap:spacing.sm},
  hero:{borderRadius:radius.xl,backgroundColor:colors.primary,padding:spacing.lg,gap:spacing.lg,...elevation.floating},heroTop:{alignItems:'flex-start',justifyContent:'space-between',gap:spacing.md},heroCopy:{flex:1,gap:spacing.xs},heroEyebrow:{color:'#CDE8DF',fontWeight:'800'},heroTitle:{color:colors.onPrimary},heroSubtitle:{color:'#DCEFE9',lineHeight:18},heroButton:{minHeight:56,borderRadius:radius.lg,backgroundColor:colors.surface,alignItems:'center',justifyContent:'space-between',paddingHorizontal:spacing.md,gap:spacing.md},heroButtonText:{color:colors.primary},heroArrow:{color:colors.primary},
