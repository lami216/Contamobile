...
type BackupFile={magic:'alkarna-mobile-backup';version:1|2|3;createdAt:string;tables:Record<string,BackupRow[]>};
const TABLES:Record<string,readonly string[]>={
  ...
  products:['id','sku','name','barcode','category_id','piece_cost','last_purchase_cost','last_purchase_at','piece_price','wholesale_price','expiry_date','note','is_archived','archived_at','created_at','updated_at'],
  ...
  account_transfers:['id','number','from_account_id','to_account_id','amount','note','occurred_at'],counters:['key','value'],app_settings:['key','value','updated_at'],audit_events:['id','action','entity_id','status','created_at'],users:['id','username','username_normalized','name','password_hash','permissions_json','is_owner','is_active','created_at','updated_at'],
};
const restoreOrder=['document_lines','stock_movements','financial_movements','account_transfers','documents','product_stocks','products','product_categories','parties','payment_accounts','warehouses','counters','app_settings','audit_events','users'];

async function buildBackup(db:SQLiteDatabase):Promise<BackupFile>{const tables:Record<string,BackupRow[]>={};for(const table of Object.keys(TABLES))tables[table]=await db.getAllAsync<BackupRow>(`SELECT * FROM ${table}`);return{magic:'alkarna-mobile-backup',version:3,createdAt:new Date().toISOString(),tables}}
...
export async function createSafetyBackup(db:SQLiteDatabase,prefix='alkarna-safety-backup'){return writeBackupFile(await buildBackup(db),prefix)}
...
export async function chooseAndRestoreBackup(db:SQLiteDatabase){const picked=await DocumentPicker.getDocumentAsync({type:'application/json',copyToCacheDirectory:true,multiple:false});if(picked.canceled)return false;const asset=picked.assets[0];if(!asset)throw new Error('No backup selected');const text=await FileSystem.readAsStringAsync(asset.uri,{encoding:FileSystem.EncodingType.UTF8});const parsed=JSON.parse(text) as Partial<BackupFile>;validateBackup(parsed);await createSafetyBackup(db,'alkarna-before-restore');await restore(db,parsed as BackupFile);await ensureAuthSchema(db);return true}
function validateBackup(value:Partial<BackupFile>){if(value.magic!=='alkarna-mobile-backup'||(value.version!==1&&value.version!==2&&value.version!==3)||!value.tables)throw new Error('ملف النسخة الاحتياطية غير صالح أو غير مدعوم');for(const table of Object.keys(TABLES)){if(table==='product_categories'&&value.version===1)continue;if(!Array.isArray(value.tables[table]))throw new Error(`النسخة لا تحتوي جدول ${table}`);for(const row of value.tables[table]??[]){for(const key of Object.keys(row))if(!TABLES[table]?.includes(key))throw new Error(`حقل غير متوقع في ${table}`)}}}
