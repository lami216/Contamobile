...
function validateBackup(value:Partial<BackupFile>){if(value.magic!=='alkarna-mobile-backup'||(value.version!==1&&value.version!==2)||!value.tables)throw new Error('ملف النسخة الاحتياطية غير صالح أو غير مدعوم');for(const table of Object.keys(TABLES)){if(!Array.isArray(value.tables[table])){if(value.version===1&&table==='product_categories'){value.tables[table]=[];continue}throw new Error(`النسخة لا تحتوي جدول ${table}`)}for(const row of value.tables[table]??[]){for(const key of Object.keys(row))if(!TABLES[table]?.includes(key))throw new Error(`حقل غير متوقع في ${table}`)}}}
