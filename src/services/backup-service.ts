...
type BackupFile={magic:'alkarna-mobile-backup';version:1|2;createdAt:string;tables:Record<string,BackupRow[]>};
...
async function buildBackup(db:SQLiteDatabase):Promise<BackupFile>{const tables:Record<string,BackupRow[]>={};for(const table of Object.keys(TABLES))tables[table]=await db.getAllAsync<BackupRow>(`SELECT * FROM ${table}`);return{magic:'alkarna-mobile-backup',version:2,createdAt:new Date().toISOString(),tables}}
